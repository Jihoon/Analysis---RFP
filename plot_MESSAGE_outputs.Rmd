---
title: "Plot MESSAGE key outputs"
...
1. Plots of Key variables from MESSAGE runs (SSP1-3) with various climate policies

1.1 Import data and rearrange for ggplot
```{r}
library(tidyverse)
library(readxl)
library(ggplot2)
...
# tiff(file = paste0("./Plot/GHG emissions.tif"), width = 1600, height = 1200)
...
# Take into account full combination of SSP/budget to show which scenarios are infeasible on the plots
...
# Long format
# final_df_l_eu = final_df %>% # By End-use sector
...
# Generate long-format table for each sub-sector
eu_l = function(sect) {
...
# Take into account full combination of SSP/budget to show which scenarios are infeasible on the plots
full_comb1 = list(SCENARIO=Scenario_order[1:6], budget=c(400, 1000, 1600), year=unique(final_df_l_cr$year), VARIABLE=unique(final_df_l_cr$VARIABLE))
...
# full_comb2 = list(SCENARIO=Scenario_order[7:15], budget=Inf, year=unique(final_df_l_cr$year))
# full_comb = rbind(do.call(expand.grid, full_comb1), do.call(expand.grid, full_comb2))
# GDP_pop_df_l = full_comb %>% left_join(GDP_pop_df_l)
```

```{r}
gp = ggplot() +
  geom_line(data = GDP_pop_df_l, aes(x=year, y=GDP_pcap, group=SCENARIO, color = Category), size=1.3, alpha = 0.7) + 
  geom_line(data = GDP_pop_df_l, aes(x=year, y=Population*7/1000, group=SCENARIO, color = Category), size=1, alpha = 0.4, linetype="twodash") + 
  facet_grid(budget~SSP) +
  labs(y="GDP per capita [US$2010 PPP/yr]") + 
  theme(axis.text.x = element_text(angle = 60, hjust = 1)) + 
  scale_y_continuous(sec.axis = sec_axis(~./7*1000, name = "Population [Billion]"))
print(gp)
```
